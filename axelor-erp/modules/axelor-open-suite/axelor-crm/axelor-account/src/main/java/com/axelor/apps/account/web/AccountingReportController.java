...
public class AccountingReportController {
  ...
  public void printExportMoveLine(ActionRequest request, ActionResponse response) {
    ...
    AccountingReportService accountingReportService = Beans.get(AccountingReportService.class);
    String bln = "";
    if (request.getContext().get("balance_b") != null)
      bln = String.valueOf(request.getContext().get("balance_b").toString());
    try {
      ...
      if ((typeSelect >= AccountingReportRepository.EXPORT_ADMINISTRATION
        ...
        if (bln.equals("balance_8") || bln.isEmpty()) {
          fileLink = accountingReportService.getReportFileLink(accountingReport, name);
        } else {

          fileLink = accountingReportService.getReportFileLinkk(accountingReport, name);
        }
