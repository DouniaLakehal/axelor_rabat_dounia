...
public class MoveValidateService {
  ...
  @Transactional
  public void validateMultipleouverturemultip(List<? extends Move> moveList) {
    for (Move move : moveList) {
  ...
  @Transactional
  public void modifierGM(GroupeMove gm) {
    gm.setStatusSelect(2);
    Period p = Beans.get(PeriodRepository.class).find(gm.getPeriod().getId());
    p.setStatusSelect(PeriodRepository.STATUS_OPENED);
    Beans.get(PeriodRepository.class).save(p);
  }
