...
public class ConvertNomreToLettres {
  ...
  public static String getStringMontant(BigDecimal Montant) {
    Montant = Montant.setScale(2, RoundingMode.HALF_UP);
    String somme_string = BigDecimal.ZERO.compareTo(Montant) > 0 ? "MOINS " : "";
    Montant = Montant.abs();
    somme_string += ConvertNomreToLettres.convert(Montant.longValue()).toUpperCase() + " DIRHAMS";
    if (Montant.compareTo(new BigDecimal(String.valueOf(Montant.longValue()))) > 0) {
      somme_string = somme_string + " ET ";
      String decimal =
          Montant.remainder(BigDecimal.ONE).movePointRight(Montant.scale()).abs().toString();
      somme_string +=
          (ConvertNomreToLettres.convert(Long.valueOf(decimal)) + " CENTIMES").toUpperCase();
    }
    return somme_string;
  }
}
