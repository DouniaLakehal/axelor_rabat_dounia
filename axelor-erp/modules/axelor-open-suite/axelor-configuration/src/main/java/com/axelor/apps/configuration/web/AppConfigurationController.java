...
public class AppConfigurationController {
	...
	@Inject
	MyConfigurationServiceImpl configurationService;
	...
	public void redirect_to_detail_depense(ActionRequest request, ActionResponse response) {
		//check id_version is present we need id_version obligatoire stp!!!
		...
		for (int i = 0; i < ls_ids.size(); i++) {
			if (ls_ids.get(i) != null) {
				...
				if (parent_budget.get(i).getCode_budget().equals("61")) {
					//kouidi
					budget_tmp = configurationService.getRubriqueBudgetaireByYearAndCode("612", 0L);
				}
	...
	public void tw_start_generate_All_rubrique(ActionRequest request, ActionResponse response) {
		...
		try {
			...
			if (!((vv != null &&
					Beans.get(VersionRubriqueBudgetaireRepository.class).all()
							.filter("self.id!=:id")
							.bind("id", vv.getId())
							.fetch().size() == 0) ||
					(Beans.get(VersionRubriqueBudgetaireRepository.class).all().fetch().size() == 0))) {
				List<RubriquesBudgetaire> ls =
	...
	public void tw_get_all_rubrique_budgetaire(ActionRequest request, ActionResponse response) {
		...
		RubriqueConfiguration _bank = configurationService.getCategorieBudgetaireByCode("5%", id_version);
		RubriqueConfiguration _exploit = configurationService.getCategorieBudgetaireByCode("7%", id_version);
		RubriqueConfiguration _resource = configurationService.getCategorieBudgetaireByCode("1%", id_version);
		RubriqueConfiguration _charges = configurationService.getCategorieBudgetaireByCode("6%", id_version);
		RubriqueConfiguration _emploi = configurationService.getCategorieBudgetaireByCode("2%", id_version);
		RubriqueConfiguration _credit = configurationService.getCategorieBudgetaireByCode("", id_version);
		response.setView(
				ActionView.define("Afficher les Budget")
						.model(RubriquesBudgetaire.class.getName())
						...
						.context("id_type_principale_banque", _bank == null ? 0L : _bank.getId())
						.context("id_type_principale_produitExploitation", _exploit == null ? 0L : _exploit.getId())
						.context("id_type_principale_ressources", _resource == null ? 0L : _resource.getId())
						.context("id_type_principale_chargesExterne", _charges == null ? 0L : _charges.getId())
						.context("id_type_principale_emploi_inves", _emploi == null ? 0L : _emploi.getId())
						.context("id_type_principale_creditNomProg", _credit == null ? 0L : _credit.getId())
	...
	public void tw_lowd_data_now(ActionRequest request, ActionResponse response) {
		...
		// tresorerie
		RubriqueConfiguration _bank = configurationService.getCategorieBudgetaireByCode("5%", version_id);
		BigDecimal some1 = configurationService.getSomme(_bank == null ? 0L : _bank.getId(), annee, version_id);
		// produit exploitation
		RubriqueConfiguration _exploit = configurationService.getCategorieBudgetaireByCode("7%", version_id);
		BigDecimal some2 = configurationService.getSomme(_exploit == null ? 0L : _exploit.getId(), annee, version_id);
		// ressource d'invesstissement
		RubriqueConfiguration _resource = configurationService.getCategorieBudgetaireByCode("1%", version_id);
		BigDecimal some3 = configurationService.getSomme(_resource == null ? 0L : _resource.getId(), annee, version_id);
		// Charges exploitation
		RubriqueConfiguration _charges = configurationService.getCategorieBudgetaireByCode("6%", version_id);
		BigDecimal some4 = configurationService.getSomme(_charges == null ? 0L : _charges.getId(), annee, version_id);
		// Emploi investissement
		RubriqueConfiguration _emploi = configurationService.getCategorieBudgetaireByCode("2%", version_id);
		BigDecimal some5 = configurationService.getSomme(_emploi == null ? 0L : _emploi.getId(), annee, version_id);
		// ressource d'invesstissement
		RubriqueConfiguration _credit = configurationService.getCategorieBudgetaireByCode("", version_id);
		BigDecimal some6 = configurationService.getSomme(_credit == null ? 0L : _credit.getId(), annee, version_id);
	...
	public void tw_get_show_rubrique_budgetaire(ActionRequest request, ActionResponse response) {
		Long id = (Long) request.getContext().get("id");
		response.setView(
				ActionView.define("Saisir les Budget")
						.model(RubriquesBudgetaire.class.getName())
						.add("grid", "gridRubriquesBudgetaireShowAll")
						.add("form", "custom_budget_info_read_only")
						.param("popup", "false")
						.context("id_version", id)
						.domain("self.id_version=" + id)
						.map());
