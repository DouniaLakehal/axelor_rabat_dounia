...
import com.axelor.apps.helpdesk.db.Reclamation;
import com.axelor.apps.helpdesk.db.Sla;
import com.axelor.apps.helpdesk.db.Ticket;
import com.axelor.apps.helpdesk.db.repo.ReclamationRepository;
...
public class TicketServiceImpl implements TicketService {
  ...
  @Inject private ReclamationRepository reclamationRepository;
  ...
  public String chargerNumeros(Long id) {
    Reclamation r = reclamationRepository.find(id);
    String code = "";
    if (r.getTicketSeqence() == null) {
      code = String.format("Rec%06d", r.getId());
    }
    return code;
  }

  @Override
  public boolean checkAllTachesReclamation(Long id) {
    boolean res = true;
    Reclamation r = reclamationRepository.find(id);
    if (r != null) {
      List<ActionsReclamation> ls = r.getActionReclamation();
      for (ActionsReclamation tmp : ls) {
        if (tmp.getTerminer() == false && res == true) {
          res = false;
        }
      }
    }
