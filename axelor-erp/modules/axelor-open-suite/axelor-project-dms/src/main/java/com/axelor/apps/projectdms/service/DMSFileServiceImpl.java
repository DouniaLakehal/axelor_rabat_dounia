...
public class DMSFileServiceImpl implements DMSFileService {
  ...
  public void sendEmail(DMSFile dmsFile) throws AxelorException, MessagingException {
    ...
    tmp.setAddress(currentUser.getEmail());
    EmailAddress emailAddress = tmp;
    ...
    if (language != null && language.equals("fr")) {
      content = String.format(CONTENT_EN, "ccisbk", currentUser.getFullName());
    } else {
      content = String.format(CONTENT_EN, "failed", currentUser.getFullName());
    }
