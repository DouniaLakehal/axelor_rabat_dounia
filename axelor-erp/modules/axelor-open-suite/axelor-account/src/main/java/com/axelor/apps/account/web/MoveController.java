...
import com.axelor.apps.account.db.GroupeMove;
...
public class MoveController {
  ...
  public void validateMultipleMovesouverture(ActionRequest request, ActionResponse response) {
    List<Long> moveIds = (List<Long>) request.getContext().get("_ids");
    if (moveIds != null && !moveIds.isEmpty()) {
      Long id = Long.valueOf(request.getContext().get("id_groupMouve").toString());
      GroupeMove gm = Beans.get(GroupeMoveRepository.class).find((Long) id);
      if (gm.getStatusSelect() == 3) {
        Beans.get(MoveService.class).getMoveValidateService().modifierGM(gm);
      }
      List<? extends Move> moveList =
          Beans.get(MoveRepository.class)
              .all()
      ...
      if (!moveList.isEmpty()) {
        boolean error = false;
        Beans.get(MoveService.class)
            .getMoveValidateService()
            .validateMultipleouverturemultip(moveList);
        if (error) response.setFlash(I18n.get(IExceptionMessage.MOVE_VALIDATION_NOT_OK));
        else {
          // response.setFlash(I18n.get(IExceptionMessage.MOVE_VALIDATION_OK));
          response.setReload(true);
  ...
  public void checkRemoveLines(ActionRequest request, ActionResponse response) {
    if (request.getContext().get("difference") != null
        && !request.getContext().get("difference").toString().equals("0")) {
      response.setError("Erreur : Les ecritures ne sont pas equilibrées");
      return;
    }
    try {
