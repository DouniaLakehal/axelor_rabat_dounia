...
public class MoveLineController {
  ...
  public void accountingReconcile(ActionRequest request, ActionResponse response) {
    ...
    String msg = check_precondition(idList);
    if (!msg.isEmpty()) {
  ...
  private String check_precondition(List<Integer> idList) {
    String res = "";
    BigDecimal sum_debit = BigDecimal.ZERO;
    BigDecimal sum_credit = BigDecimal.ZERO;
    for (Integer _id : idList) {
      MoveLine m = Beans.get(MoveLineRepository.class).find(Long.valueOf(_id));
      sum_credit = sum_credit.add(m.getCredit());
      sum_debit = sum_debit.add(m.getDebit());
    }
    if (sum_credit.compareTo(sum_debit) != 0) {
      res = "La somme des débit est différent de la somme de crédit";
    }
    return res;
  }
