...
public class AccountingReportServiceImpl implements AccountingReportService {
  ...
  public String getReportFileLink(AccountingReport accountingReport, String name)
    ...
    if (accountingReport.getTypeSelect() == 11 && accountingReport.getJournal() == null) {
      reportSettings =
          ReportFactory.createReport(IReport.ACCOUNTING_REPORT_TYPE_JOURNAL, name + "-${date}")
  ...
  @Override
  public String getReportFileLinkk(AccountingReport accountingReport, String name)
      throws AxelorException {
    return ReportFactory.createReport(
            String.format(IReport.ACCOUNTING_REPORT_TYPE1, accountingReport.getTypeSelect()),
            name + "-${date}")
        .addParam("AccountingReportId", accountingReport.getId())
        .addParam("Locale", ReportSettings.getPrintingLocale(null))
        .addParam(
