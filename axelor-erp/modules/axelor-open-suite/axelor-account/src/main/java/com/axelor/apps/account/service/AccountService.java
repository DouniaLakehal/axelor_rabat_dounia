...
public class AccountService {
  ...
  public BigDecimal computeBalance_v2(Account account, int balanceType, GroupeMove gm) {

    Query balanceQuery =
        JPA.em()
            .createQuery(
                "select sum(self.debit - self.credit) from MoveLine self where self.account = :account "
                    + "and self.move.ignoreInAccountingOk IN ('false', null) and self.move.id in (:ids_move) "
                    + "AND self.move.journal.id_journal_parent!= :id_journ_anv ");

    balanceQuery.setParameter("account", account);
    balanceQuery.setParameter(
        "ids_move", gm.getMove_move().stream().map(Move::getId).collect(Collectors.toList()));
    balanceQuery.setParameter(
        "id_journ_anv",
        Beans.get(JournalRepository.class)
            .all()
            .filter("self.is_journalAnouv is true")
            .fetchOne()
            .getId());
