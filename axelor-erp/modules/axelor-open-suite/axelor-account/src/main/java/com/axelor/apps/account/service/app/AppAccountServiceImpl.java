...
public class AppAccountServiceImpl extends AppBaseServiceImpl implements AppAccountService {
  ...
  public void AnnulerEncaissement(Long id, String commentaire) {
    ...
    ec.setStatut("Annuler");
    ec.setMontant(BigDecimal.valueOf(0));
    encaissementRepository.save(ec);
  }

  @Override
  @Transactional
  public void ModifierEncaissement(Long id) {
    Encaissement ec = getEncaissementById(id);
    encaissementRepository.save(ec);
  }
  ...
  @Override
  @Transactional
  public void VerserEncaissement(Long id) {
    Encaissement ec = getEncaissementById(id);
    if (Objects.equals(ec.getStatut(), "Annuler")) {
      ec.setStatut("Annuler");
    } else {
      ec.setStatut("Verser");
      ec.setEtat("F");
  ...
  public String getNumeroQuittanceInc() {
    List<Encaissement> lls = encaissementRepository.all().fetch();
    Encaissement enc =
    ...
    String res = "";
    if (enc != null) {
      res = (Integer.parseInt(enc.getNumero().split("/")[0]) + 1) + "/" + LocalDate.now().getYear();
    } else {
      res = "1/" + LocalDate.now().getYear();
    }
    return res;
  }

  @Transactional
  public Boolean getNumeroOV() {
    ...
    if (lls.compareTo(BigDecimal.valueOf(5000)) >= 0 || lls.equals(BigDecimal.valueOf(0))) {
      for (Encaissement tmp : encaissementRepository.all().filter("self.etat='E'").fetch()) {
        tmp.setEtat("F");
        encaissementRepository.save(tmp);
      }
      avance = true;
      return avance;
    }
  ...
  public String getNumeroOrdre() {
    String ans = String.valueOf(LocalDate.now().getYear());
    List<GestionRecette> lls =
        gestionRecetteRepository.all().filter("self.ordre is not null").fetch();
    String res = "";
    if (lls.size() > 0) {
      res =
          (Integer.parseInt(lls.get(lls.size() - 1).getOrdre().split("/")[0]) + 1)
              + "/"
              + ans.substring(ans.length() - 2);
    } else {
      res = "1/" + ans.substring(ans.length() - 2);
    }
    return res;
  }
  ...
  public AccountPrintReconcil save_account_print_reconcil(AccountPrintReconcil ac) {
    return Beans.get(AccountPrintReconcilRepository.class).save(ac);
  }

  @Override
  @Transactional
  public void saveEncaissement(Encaissement o) {
    encaissementRepository.save(o);
  }

  @Override
  @Transactional
  public TresaurerieAnnuel saveTresairerie(TresaurerieAnnuel temp) {
    return Beans.get(TresaurerieAnnuelRepository.class).save(temp);
  }
