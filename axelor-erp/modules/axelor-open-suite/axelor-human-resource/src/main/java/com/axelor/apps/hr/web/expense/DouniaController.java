...
public class DouniaController {
  ...
  @Inject OrdreVirementRepository ordreVirementRepository;
  ...
  public void OrdrePaiement(ActionRequest request, ActionResponse response) throws AxelorException {
    ...
    String req_ind =
        "select sum(es.indemnite_logem_brut) as logement,\n"
            + "       sum(es.indemnit_fonction_net) as fonction,\n"
    ...
    float somme_ind = ((BigDecimal) obj_ind[4]).floatValue();
    BigDecimal somme_ind2 = ((BigDecimal) obj_ind[4]);
    float somme_net =
        (x).floatValue()
            + ((BigDecimal) obj_ind[1]).floatValue()
            + ((BigDecimal) obj_ind[2]).floatValue()
            + ((BigDecimal) obj_ind[3]).floatValue();
    ...
    float net = ((BigDecimal) obj_ind[4]).floatValue() - somme_net;
    ...
    javax.persistence.Query req_cnss =
        JPA.em()
            .createNativeQuery(
                "select sum(he.montant_cnss) as cnss from hr_etat_salaire es\n"
                    + "                                left join hr_employee he on es.employee = he.id where es.mois=?1 and es.annee=?2")
            .setParameter(1, mois)
            .setParameter(2, annee);
    Object etat_cnss = req_cnss.getSingleResult();
    ...
    String fileLink =
        ReportFactory.createReport(
            ...
            .addParam("logement", x)
            .addParam("fonction", obj_ind[1])
            .addParam("voiture", obj_ind[2])
            .addParam("represent", obj_ind[3])
            .addParam("salaires", obj_ind[4])
  ...
  public void ReleveBancaire(ActionRequest request, ActionResponse response)
    ...
    float somme_amo = ((BigDecimal) etat_amo).floatValue();
    float total_amo = ((BigDecimal) etat_amo).floatValue() + ((BigDecimal) etat_amo).floatValue();
    ...
    javax.persistence.Query rel_eqdom =
        JPA.em()
            .createNativeQuery(
                "select sum(gc.remboursement) as eqdom from hr_etat_salaire es\n"
                    + "                                left join hr_employee he on es.employee = he.id\n"
                    + "                                left join HR_GESTION_CREDIT gc on he.id = gc.employee where gc.intituler=3 and es.mois=?1 and es.annee=?2 and he.id in(132,146)")
            .setParameter(1, mois)
            .setParameter(2, annee);
