...
import com.axelor.apps.configuration.db.repo.AOS_configRepository;
...
import com.axelor.apps.hr.db.repo.RappelEmployeRepository;
...
@Singleton
public class EtatSalaireServiceImpl extends AppBaseServiceImpl implements EtatSalaireService {
  ...
  @Transactional
  public List<EtatSalaire> AddDataToEtatSalaire(int annee, int mois) {
    String dateDebut1 = "";
    List<EtatSalaire> ls = new ArrayList<>();
    if (mois >= 10) {
      dateDebut1 = annee + "-" + mois + "-" + "01";
    } else {
      dateDebut1 = annee + "-0" + mois + "-" + "01";
    }
  ...
  @Transactional
  public void AddPersonalEmployeeInfo(
    ...
    etatSalaire.setTitulaire(employee.getTypePersonnel() == false || employee.getTypePersonnel() == null);
    // situation familiale
    ...
    Disposition disposition = getDispositionByEmployee(employee, dateFinMois);
    if (disposition == null) {
    ...
    BigDecimal totalBrut = etatSalaire.getTraitBaseIndemStatu().setScale(2, RoundingMode.HALF_UP);
    ...
    if (!employee.getTypePersonnel()
            && getDispositionByEmployee(employee, dateFinMois) == null) {
      if (totalBrut
    ...
    BigDecimal total_2_old = totalBrut.subtract(indem_log_old);
    if (getDispositionByEmployee(employee, dateFinMois) == null) {
    ...
    total_x_old = totalBrut.multiply(tva.divide(_prcent)).setScale(2, RoundingMode.HALF_UP);
  ...
  private void setreponsabiliteNull(EtatSalaire etatSalaire) {
  ...
  public int getNumberOfEtatSalaire(int annee, int mois) {
  ...
  public BigDecimal calculMGPAP(BigDecimal total_2_old, MUTUELLE sec) {
    BigDecimal _prcent = new BigDecimal("100");
  ...
  public boolean employer_has_credit(Employee employee) {
    boolean res = false;
  ...
  @Transactional
  public void supprimerTousEtatSalaire(int annee, int mois) {
  ...
  public Disposition getDispositionByEmployee(Employee employee, String dateEtat) {
