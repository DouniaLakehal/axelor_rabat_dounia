...
public class CampaignController {
  ...
  public void tw_genererJoutnalAnouv(ActionRequest request, ActionResponse response) {
    Long id = (Long) request.getContext().get("id");
    GroupeMove gm = Beans.get(CustomService.class).getGroupeMoveById(id);
    List<Move> moveList = gm.getMove_move();

    boolean is_move_hase_journal_an = false;
    for (Move m : moveList) {
      if (!is_move_hase_journal_an
          && Beans.get(JournalRepository.class)
              .find(m.getJournal().getId_journal_parent())
              .getIs_journalAnouv()) is_move_hase_journal_an = true;
    }

    if (!is_move_hase_journal_an) {
      Beans.get(CustomService.class).addJournaleRN(gm);
      Beans.get(GroupeMoveRepository.class).find(gm.getId());
    }
  }
}
