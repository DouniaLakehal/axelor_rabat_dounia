...
public class PartnerController {
  ...
  public void checkIbanValidity(ActionRequest request, ActionResponse response)
    ...
    if ((((boolean) request.getContext().get("isCustomer"))
            || ((boolean) request.getContext().get("isSupplier")))
        && request.getContext().get("partnerCode") == null) {
      List<Partner> lsp =
          Beans.get(PartnerRepository.class).all().filter("self.isContact!=null").fetch();
      int nbr =
          lsp.stream()
              .filter(
                  oo -> {
                    if ((boolean) request.getContext().get("isCustomer")) return oo.getIsCustomer();
                    else return oo.getIsSupplier();
                  })
              .collect(Collectors.toList())
              .size();
      String code =
          (boolean) request.getContext().get("isCustomer")
              ? String.format("C%03d", nbr + 1)
              : String.format("F%03d", nbr + 1);
      response.setValue("partnerCode", code);
    }
