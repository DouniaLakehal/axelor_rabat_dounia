...
import com.axelor.apps.configuration.db.Armoire;
import com.axelor.apps.contract.db.*;
...
public class ContractController {
  ...
  @Inject ContractService appService;
  ...
  public void AddArmoire(ActionRequest request, ActionResponse response) {
    Armoire armoire = request.getContext().asType(Armoire.class);

    try {
      if (armoire.getIsCouloir()) {
        armoire.setBureau(null);

      } else if (armoire.getIsBureau()) {
        armoire.setCouloir(null);
      }
      response.setValues(armoire);
    } catch (Exception e) {
      TraceBackService.trace(response, e, ResponseMessageType.ERROR);
    }
  }

  public void theadresse(ActionRequest request, ActionResponse response) {
    Expediteur expediteur = (Expediteur) request.getContext().get("nomExpediteur");
    response.setValue("adresseDuDestinataire", expediteur.getAdresee());
    response.setValue("adresseMailDestinataire", expediteur.getEmail());
  }

  public void theemail(ActionRequest request, ActionResponse response) {
    Expediteur expediteur = (Expediteur) request.getContext().get("nomExpediteur");
    response.setValue("adresseDuExpedit", expediteur.getAdresee());
    response.setValue("adresseMailExpedit", expediteur.getEmail());
  }

  public void normalizePhoneNumber(ActionRequest request, ActionResponse response) {
    ContractService annexeService = Beans.get(ContractService.class);
    try {
      String phoneNumberFieldName = annexeService.getPhoneNumberFieldName(request.getAction());
      String phoneNumber = (String) request.getContext().get(phoneNumberFieldName);

      if (!StringUtils.isBlank(phoneNumber)) {
        String normalizedPhoneNumber = annexeService.normalizePhoneNumber(phoneNumber);

        if (!phoneNumber.equals(normalizedPhoneNumber)) {
          response.setValue(phoneNumberFieldName, normalizedPhoneNumber);
        }

        if (!annexeService.checkPhoneNumber(normalizedPhoneNumber)) {
