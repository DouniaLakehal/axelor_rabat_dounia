...
import com.axelor.apps.contract.db.repo.*;
...
public class ContractServiceImpl extends ContractRepository implements ContractService {
  ...
  protected AbstractCourrierRepository courrierRepository;
  ...
  @Transactional
  public void updateNbr_jour() {
    List<Courrier> ls = courrierRepository.all().fetch();
    for (Courrier c : ls) {
      LocalDate now = LocalDate.now();
      LocalDate reception = c.getDateReception();
      LocalDate limite = c.getDateLimite();

      int x = reception.compareTo(now);
    }
  }
}
