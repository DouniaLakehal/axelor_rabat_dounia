...
import com.axelor.apps.account.db.DepencesGenerale;
import com.axelor.apps.account.db.RecetteGenerale;
import com.axelor.apps.account.db.RecetteGeneraleGrand;
...
import com.axelor.apps.account.db.repo.RecetteGeneraleRepository;
...
import com.axelor.apps.configuration.db.repo.ListRubriqueDepensesGeneraleRepository;
...
public class PurchaseOrderServiceImpl implements PurchaseOrderService {
  ...
  @Override
  @Transactional
  public RecetteGeneraleGrand dupliquerRecette(RecetteGeneraleGrand r_prec, int annee)
      throws Exception {
    RecetteGeneraleGrand ma_copie = new RecetteGeneraleGrand();
    ma_copie.setAnnee(annee);
    Set<RecetteGenerale> ls = new HashSet<>();
    for (RecetteGenerale tmp : r_prec.getBudgetrea()) {
      RecetteGenerale _tmp = Beans.get(RecetteGeneraleRepository.class).copy(tmp, true);
      RubriquesBudgetaire rb =
          Beans.get(ServiceUtil.class)
              .getRubriqueBudgetaireGeneraleByAnneeAndCode(annee, tmp.getBudgetrub().getCodeBudg());
      RubriqueBudgetaireGenerale to_find = null;
      if (rb != null) {
        to_find =
            Beans.get(RubriqueBudgetaireGeneraleRepository.class)
                .find(rb.getId_rubrique_generale());
      }
      _tmp.setBudgetrub(to_find);
      _tmp.setRealisations(BigDecimal.ZERO);
      ls.add(Beans.get(RecetteGeneraleRepository.class).save(_tmp));
    }
    ma_copie.setBudgetrea(ls);
    ma_copie = Beans.get(RecetteGeneraleGrandRepository.class).save(ma_copie);
    return ma_copie;
  }

  @Override
  @Transactional
  public DepencesGenerale dupliquerDepenses(DepencesGenerale dp_prec, int annee) throws Exception {
    DepencesGenerale ma_copie = new DepencesGenerale();
    ma_copie.setYear(annee);

    Set<ListRubriqueDepensesGenerale> ls = new HashSet<>();
    for (ListRubriqueDepensesGenerale tmp : dp_prec.getListRubriqueDepensesGenerale()) {
      ListRubriqueDepensesGenerale _tmp =
          Beans.get(ListRubriqueDepensesGeneraleRepository.class).copy(tmp, true);
      RubriquesBudgetaire rb =
          Beans.get(ServiceUtil.class)
              .getRubriqueBudgetaireGeneraleByAnneeAndCode(
                  annee, tmp.getRubriquesBudgetaire().getCodeBudg());
      RubriqueBudgetaireGenerale to_find = null;
      if (rb != null) {
        to_find =
            Beans.get(RubriqueBudgetaireGeneraleRepository.class)
      ...
      _tmp.setRubriquesBudgetaire(to_find);
      ls.add(_tmp);
    }
    ma_copie.setListRubriqueDepensesGenerale(ls);
    ma_copie = Beans.get(DepencesGeneraleRepository.class).save(ma_copie);
    return ma_copie;
  }
}
